#====================================================================
# File Name: tdms_reader_time.py
# Project Name: dataprocess
# Description:
# 1、读取TDMS文件：遍历原始数据目录下的所有子文件
# 2、生成CSV文件：文件名保持与原TDMS文件相同的时间戳格式
# 3、CSV文件结构：
#   列名         数据类型                   描述
#   time         字符串         相对时间，从0开始的秒数，格式为浮点数
#   channel1     数值?(float)    通道1的传感器数据
#   channel2     数值?(float)    通道2的传感器数据
#   ...          ...             ...
#   channel16    数值?(float)    通道16的传感器数据
# 4、同时生成对应的元数据JSON文件，包含采样信息和时间参考
# 5、采样间隔：2.5微秒 (400kHz采样率)，从TDMS文件属性自动读取
# 6、支持多进程并行处理，提高大文件处理效率
#====================================================================

import numpy as np
from nptdms import TdmsFile
import os
import pandas as pd
from datetime import datetime, timedelta
import re
from concurrent.futures import ProcessPoolExecutor
import multiprocessing
import yaml
import sys
from pathlib import Path
import logging
import json 

from src.utils.perf_timing import add_counter, record_file_size, timed_step

logger = logging.getLogger('data_process')


def load_config():
    project_root = Path(__file__).resolve().parent.parent.parent
    config_path = project_root / "config" / "paths.yaml"
    with open(config_path, 'r', encoding='utf-8') as file:
        config = yaml.safe_load(file)
    logger.info(f"配置文件加载成功: {config_path}")
    return config

def process_tdms_file(tdms_path, output_base_dir, raw_data_dir):
    """Process one TDMS file and export CSV + metadata."""
    try:
        filename = os.path.basename(tdms_path)
        time_str = re.search(r'(\d{8}\d{6}\.\d+)', filename)
        start_time_str = time_str.group(1) if time_str else None

        with timed_step('timedata.tdms_read', file=filename):
            tdms_file = TdmsFile.read(tdms_path)
        groups = tdms_file.groups()
        
        group = groups[0]
        channels = [ch for ch in group.channels()]
            
        data_length = len(channels[0])
        num_channels = len(channels)
        
        first_channel = channels[0]
        
        # 直接读取 channel.wf_increment 原始值
        wf_increment = float(first_channel.properties['wf_increment'])
        sampling_interval = wf_increment
        sampling_rate_hz = 1.0 / sampling_interval
        logger.info(f"sampling interval from wf_increment: {sampling_interval:.15g} s")

        # 使用 TDMS 内部的 wf_start_time 作为 start_time（精确同步）
        wf_start_time = first_channel.properties['wf_start_time']
        start_time_absolute = wf_start_time.astype('M8[us]').astype('O')
        
        time_relative_seconds = np.arange(data_length) * sampling_interval
        time_relative_ms = time_relative_seconds * 1000
        time_relative_us = time_relative_seconds * 1e6
        time_list = [f"{t:.9f}" for t in time_relative_seconds]
        
        data_dict = {"time": time_list}
        for i, channel in enumerate(channels):
            data_dict[f"channel{i+1}"] = channel[:]
        
        df = pd.DataFrame(data_dict)
        
        relative_path = os.path.relpath(os.path.dirname(tdms_path), raw_data_dir)
        output_dir = os.path.join(output_base_dir, relative_path)
        os.makedirs(output_dir, exist_ok=True)
        
        # CSV 文件名使用 wf_start_time 格式化的时间字符串
        start_time_for_filename = start_time_absolute.strftime("%Y%m%d%H%M%S.%f")
        output_filename = f"{start_time_for_filename}.csv"
        output_path = os.path.join(output_dir, output_filename)
        
        chunk_size = 100000
        with timed_step('timedata.csv_write', file=output_filename, rows=data_length):
            for i in range(0, data_length, chunk_size):
                end_idx = min(i + chunk_size, data_length)
                chunk_df = df.iloc[i:end_idx]

                if i == 0:
                    chunk_df.to_csv(output_path, index=False, encoding='utf-8-sig', float_format='%.9f')
                else:
                    chunk_df.to_csv(output_path, mode='a', header=False, index=False, encoding='utf-8-sig', float_format='%.9f')
        add_counter('timedata.output_files', 1)
        add_counter('timedata.output_rows', data_length)
        record_file_size(output_path, 'timedata.output_bytes')
            
        # === 保存元数据文件 ===
        # 获取通道名称映射
        channel_names = {}
        for i, ch in enumerate(channels):
            ni_channel_name = ch.properties.get('NI_ChannelName', ch.name)
            channel_names[f'channel{i+1}'] = ni_channel_name
        
        metadata = {
            'filename': filename,
            'start_time': start_time_absolute.isoformat(),
            'sampling_interval_seconds': sampling_interval,
            'sampling_rate_hz': sampling_rate_hz,
            'data_length': data_length,
            'total_duration_seconds': round(data_length * sampling_interval, 10),
            'num_channels': num_channels,
            'channel_names': channel_names,
        }
        
        # 保存元数据为JSON
        metadata_path = output_path.replace('.csv', '_metadata.json')
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(metadata, f, indent=2, ensure_ascii=False)
        add_counter('timedata.output_files', 1)
        record_file_size(metadata_path, 'timedata.output_bytes')
        
    except Exception as e:
        logger.error(f"处理文件 {os.path.basename(tdms_path)} 时出错: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())

def process_tdms_file_wrapper(args):
    """Wrapper for multiprocessing."""
    tdms_path, output_base_dir, raw_data_dir = args
    from src.utils.logging_utils import setup_logging
    import os
    project_root = Path(__file__).resolve().parent.parent.parent
    log_config_path = project_root / "config" / "logging.yaml"
    if os.path.exists(log_config_path):
        setup_logging(log_config_path)
    return process_tdms_file(tdms_path, output_base_dir, raw_data_dir)

def process_tdms_files_parallel(tdms_files, output_base_dir, raw_data_dir):
    max_workers = multiprocessing.cpu_count()
    logger.info("TIMING timedata.workers count=%s files=%s", max_workers, len(tdms_files))
    params = [(tdms_file, output_base_dir, raw_data_dir) for tdms_file in tdms_files]
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        list(executor.map(process_tdms_file_wrapper, params))

def resolve_scan_folder(base_dir, data_folder=None):
    """Resolve scan folder; support absolute or relative path."""
    base_path = Path(base_dir).resolve()
    if not data_folder:
        return str(base_path)

    selected_path = Path(data_folder)
    if not selected_path.is_absolute():
        selected_path = base_path / selected_path
    selected_path = selected_path.resolve()

    if not selected_path.exists():
        logger.error(f"data folder does not exist: {selected_path}")
        return None
    if not selected_path.is_dir():
        logger.error(f"data folder is not a directory: {selected_path}")
        return None

    return str(selected_path)

def main(data_folder=None):
    with timed_step('timedata.module_total'):
        config = load_config()
        if not config:
            logger.error("配置文件读取失败")

        raw_data_dir = config["raw_data_dir"]
        output_base_dir = config["tdms_reader_time_output_dir"]
        scan_folder = resolve_scan_folder(raw_data_dir, data_folder)
        if not scan_folder:
            return

        logger.info(f"读取数据目录: {raw_data_dir}")
        logger.info(f"扫描目录: {scan_folder}")
        logger.info(f"输出目录: {output_base_dir}")

        tdms_files = []
        with timed_step('timedata.file_scan', folder=scan_folder):
            for root, dirs, files in os.walk(scan_folder):
                for file in files:
                    if file.endswith('.tdms'):
                        tdms_files.append(os.path.join(root, file))
        add_counter('timedata.input_files', len(tdms_files))
        if not tdms_files:
            logger.warning("未找到可处理的TDMS文件")
            return

        logger.info(f"共找到 {len(tdms_files)} 个TDMS文件")

        process_tdms_files_parallel(tdms_files, output_base_dir, raw_data_dir)

        logger.info("TDMS转CSV处理完成")
if __name__ == "__main__":
    main()
